...
public class ParameterValidator {
    ...
    public static void validWd(String wd) {
        Assert.isTrue(wd == null || wd.length() < 64, "[关键字]过长");
    }

    public static void validStamp(Long startStamp, Long endStamp) {
//        int i = 0;
//        i += startStamp == null ? 0 : 1;
//        i += endStamp == null ? 0 : 1;
//        Assert.isTrue(i == 0 || i == 2, "日期参数不合法");
        if (startStamp != null && endStamp != null) {
            Assert.isTrue((endStamp - startStamp) / 3600 / 24 <= 180L, "日期跨度太大");
        }
    }

    public static void validUri(String netBackpicUri, boolean allowNull) {
        ValidUtil.validByRegex("URI", netBackpicUri, allowNull, "^/[0-9a-z]{32}\\.[a-z]+$");
    }
